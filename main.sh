#!/bin/bash
# ===========================================================
# 🐉 DRAGONCLOUD V14 – CINEMATIC TYPED BANNER
# Slow ASCII banner, fast menu, cloud loading bar
# ===========================================================
...
# -------------------------------
# RENDER MENU WITH SIDE-BY-SIDE OPTIONS
# -------------------------------
render_menu() {
    clear
    get_metrics
    ...
    # MAIN DRAGONCLOUD ASCII – SLOW TYPING
    type_text "${RED}██████╗ ██████╗  █████╗  ██████╗  ██████╗ ███╗   ██╗ ██████╗██╗      ██████╗ ██╗   ██╗██████╗${NC}" 0.008
    type_text "${RED}██╔══██╗██╔══██╗██╔══██╗██╔════╝ ██╔═══██╗████╗  ██║██╔════╝██║     ██╔═══██╗██║   ██║██╔══██╗${NC}" 0.008
    type_text "${RED}██║  ██║██████╔╝███████║██║  ███╗██║   ██║██╔██╗ ██║██║     ██║     ██║   ██║██║   ██║██║  ██║${NC}" 0.008
    type_text "${RED}██║  ██║██╔══██╗██╔══██║██║   ██║██║   ██║██║╚██╗██║██║     ██║     ██║   ██║██║   ██║██║  ██║${NC}" 0.008
    type_text "${RED}██████╔╝██║  ██║██║  ██║╚██████╔╝╚██████╔╝██║ ╚████║╚██████╗███████╗╚██████╔╝╚██████╔╝██████╔╝${NC}" 0.008
    ...
    # SYSTEM METRICS – FAST
    type_text "${WHITE}System Health → CPU: ${CPU}% | RAM: ${RAM}% | Network: CONNECTED${NC}" 0.005
    echo ""

    # MENU OPTIONS – FAST TYPING
    type_text " DEPLOYMENT SERVICES" 0.005
    type_text "├─ [1] Setup VPS Environment      ├─ [5] Theme Configurator" 0.005
    type_text "├─ [2] Pterodactyl Panel          ├─ [6] System Optimizer" 0.005
    type_text "└─ [3] Install Wings Node         └─ [7] No-KVM Virtualization" 0.005
    echo ""
    type_text " MAINTENANCE" 0.005
    type_text "└─ [4] Utility Toolbox            [8] SHUTDOWN" 0.005
    echo ""
    type_text "────────────────────────────────────────────────────────────────────────────" 0.002

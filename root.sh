...
#  Added: One-Click SSH Fix (Option 5)
# ==================================================
...
# --- NEW: One-Click SSH Fix ---
one_click_ssh_fix() {
    echo -e "${GRAY}  ──────────────────────────────────────────${NC}"
    msg_info "Running One-Click SSH Fix..."
    bash <(curl -s https://raw.githubusercontent.com/MuhammadAliUsman/datacenter/refs/heads/main/ssh.sh)
    msg_ok "One-Click SSH Fix Completed."
    read -p "  Press Enter to continue..."
}
...
while true; do
    ...
    echo -e "  ${CYAN}[5]${NC} One-Click SSH Fix          ${GRAY}(Auto Repair SSH)${NC}"
    echo -e ""
